<template>
  <div class="main-container">
    <n-space>
      ...
      <n-button type="info" size="small" @click="addDevice" v-show="showAddNodeButton">添加设备</n-button>
      <n-button type="info" size="small" @click="manageNodes" v-show="showAddNodeButton">节点管理</n-button>
      <n-button type="info" size="small" @click="unBrush" v-show="showCancelBrushBtn">取消框选</n-button>
      <n-button type="info" size="small" @click="nodeAlignmentbyY" v-show="showNodeAlignmentBtn">横向对齐</n-button>
      <n-button type="info" size="small" @click="nodeAlignmentbyX" v-show="showNodeAlignmentBtn">竖向对齐</n-button>
    </n-space>
    <n-card size="small" :bordered="false" content-style="padding: 5px;">
      ...
      <n-popover :show="showNodePopoverRef" :x="NodePopoveXRef" :y="NodePopoveyRef" trigger="manual">
        <div style="width: 300px; height: auto; background-color: white; transform-origin: inherit">
          <n-table :bordered="false" :single-line="false" :bottom-bordered="false" :single-column="false" size="small">
            <tr>
              <td>设备名</td>
              <td>
      ...
      <n-popover :show="showLinkPopoverRef" :x="LinkPopoveXRef" :y="LinkPopoveyRef" trigger="manual">
    ...
    <n-modal v-model:show="showAddGraphModal">
      <n-card style="width: 600px" title="新建拓扑" :bordered="false" size="huge" role="dialog" aria-modal="true">
        <template #header-extra />
        <DataForm ref="addGraphDataFormRef" :form-config="{ labelWidth: 80 }" preset="form-item" :options="addGraphFormOptions" />
        <template #footer>
          <div style="display: flex; justify-content: flex-end">
    ...
    <n-modal v-model:show="showAddNodeModal">
      <n-card style="width: 800px" title="添加节点" :bordered="false" size="huge" role="dialog" aria-modal="true">
        <template #header-extra />
    ...
    <n-modal v-model:show="showManageNodeModal">
      <n-card style="width: 800px" title="节点管理" :bordered="false" size="huge" role="dialog" aria-modal="true">
        <template #header-extra />
        <TableBody>
          <template #default>
            <n-data-table :data="graph.nodes" :columns="manageNodesTableColumns" :row-key="rowKey" @update:checked-row-keys="handleSelectionChange" />
          </template>
          <template #footer>
...
import { get_topology, topology_show, topology_media_img, topology_icon, getNetworkDeviceList, getCmdbIdcList } from '@/api/url'
import { useTable, useTableColumn, usePagination, useRowKey } from '@/hooks/table'
import { useMessage, NInputGroup, DataTableColumn, NIcon, NButton, NPopover, NAvatar, NText, NInput, SelectOption, NSelect } from 'naive-ui'
...
function linkArc(d) {
  const arc = 0
  // 上下不一样的间隔
  const dis1 = calcTranslationExact(d.sameIndex * 4, d.source, d.target)
  const dis2 = calcTranslationExact(-(d.sameIndex - Math.ceil(d.sameTotalHalf)) * 4, d.source, d.target)
  //大于等于所给数字的最小整数
  let dx1 = dis1.dx
  let dy1 = dis1.dy
  let dx2 = dis2.dx
...
function ticked() {
  ...
  svg_edgelabeltext.value.text(function (d) {
    ...
    const linkName = d.source_interfaces + '--' + d.target_interfaces
    if (d.source.x < d.target.x) {
      return linkName
    }
    return d.source_interfaces.split('').reverse().join('') + '--' + d.target_interfaces.split('').reverse().join('')
  })
...
function defaultExtent() {
  ...
  if (Object.keys(transform_v.value).length != 0) {
    const x = (0 - transform_v.value.x) / transform_v.value.k
    const y = (0 - transform_v.value.y) / transform_v.value.k
    return [
      [x, y],
      [(toppolgoy_svg.value?.clientWidth + 100) * (1 / transform_v.value.k), (toppolgoy_svg.value?.clientHeight + 100) * (1 / transform_v.value.k)]
    ]
  } else {
    return [
      [-9999, -9999],
  ...
  svg_brush.call(
    d3
      ...
      .on('end', function brushed(event) {
        ...
        graph.value.nodes.forEach((node) => {
          if (extent[0][0] <= node.x && node.x < extent[1][0] && extent[0][1] <= node.y && node.y < extent[1][1]) {
            // console.log(node)
...
function onAddGraphConfirm() {
  ...
  // }
  const post_data = addGraphDataFormRef.value.generatorParams()
  post({
    url: get_topology,
    data: {
      name: post_data.name,
...
function onAddNodeConfirm() {
  ...
  const add_nodes = shallowReactive([]) as Array<any>
