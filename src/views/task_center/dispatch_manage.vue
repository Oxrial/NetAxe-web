<template>
  <div class="main-container">
    <div style="width: 50%; float: left; padding: 10px">
      ...
      <n-data-table
        :data="crontab_list.slice((crontab_page - 1) * crontab_pageSize, crontab_page * crontab_pageSize)"
        :columns="crontab_tableColumns"
        :row-key="rowKey"
      />
      <div class="flex justify-center">
        <n-pagination v-model:page="crontab_page" :page-count="crontab_pageCount" show-size-picker :page-sizes="crontab_pageSizes">
          <template #prefix="{ itemCount, startIndex }"> 共 {{ crontab_count }} 项 </template>
        </n-pagination>
      </div>
    </div>
    <div style="width: 50%; float: right; padding: 10px">
      ...
      <n-data-table
        :data="interval_list.slice((interval_page - 1) * interval_pageSize, interval_page * interval_pageSize)"
        :columns="interval_tableColumns"
        :row-key="rowKey"
      />
      <div class="flex justify-center">
        <n-pagination v-model:page="interval_page" :page-count="interval_pageCount" show-size-picker :page-sizes="interval_pageSizes">
          <template #prefix="{ itemCount, startIndex }"> 共 {{ interval_count }} 项 </template>
        </n-pagination>
      </div>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    function onDataFormConfirm() {
      if (itemDataFormRef.value?.validator()) {
        ...
        naiveDailog.success({
          title: '提示',
          positiveText: '确定',
          content: '模拟部门添加/编辑成功，数据为：' + JSON.stringify(itemDataFormRef.value.generatorParams())
        })
      }
    }
    ...
    function delete_crontab(item) {
      ...
      const delete_formdata = new FormData()
    ...
    function AddCrontabConfirm() {
      ...
      const post_params = add_crontab_form.value
      const new_crontab_formdata = new FormData()
    ...
    function AddIntervalConfirm() {
      ...
      const post_params = add_interval_form.value
      const new_crontab_formdata = new FormData()
