  ...
  <n-space vertical>
    ...
    >
      <div class="form-wrapper" style="width: 95%">
        <DataForm ref="dataForm" preset="grid-item" :options="formItems" :form-config="formConfig" />
        <div class="flex justify-end">
          <n-button :loading="submitLoading" type="primary" size="small" @click="submit"> 提交 </n-button>
        </div>
      </div>
    </n-card>
    <n-card
...
import { renderCheckbox, renderCheckboxGroup, renderDatePicker, renderInput, renderPopSelect, renderSelect, renderTimePicker } from '@/hooks/form'
    ...
    function submit() {
      if (dataForm.value?.validator()) {
        ...
        setTimeout(() => {
          ...
          message.success('提交成功，参数为：' + JSON.stringify(dataForm.value?.generatorParams()))
        }, 3000)
      }
    }
    const formItems = [
      {
        ...
        required: true,
        value: ref(null),
        render: (formItem) => renderInput(formItem.value, { placeholder: '请输入会议名称', clearable: true }),
      ...
      },
      {
        ...
        key: 'content',
        value: ref(null),
        render: (formItem) => renderInput(formItem.value, { placeholder: '请输入会议内容', maxlength: 10 })
      },
      {
        label: '起止日期',
        key: 'startEndDate',
