<template>
  <div class="main-container">
    ...
    <ModalDialog ref="modalDialog" title="资产数据维护" :segmented="segmented" display-directive="show" @confirm="EditConfirm">
      <template #content>
        <DataForm ref="EditDataFormRef" :form-config="{ labelWidth: 60 }" preset="grid-item" :options="EditFormOptions" />
      </template>
    ...
    <ModalDialog ref="device_import_modalDialog" title="资产数据录入" @confirm="importDataFormConfirm" :style="{ height: '500px' }">
      <template #content>
        <DataForm
    ...
    <ModalDialog ref="ChangeLogmodalDialog" title="变更轨迹" :style="{ height: '768px', width: '1366px' }">
      <template #content>
        <n-data-table
          :data="change_log_list.slice((change_log_page - 1) * change_log_pageSize, change_log_page * change_log_pageSize)"
          size="small"
        ...
        <div class="flex justify-center">
          <n-pagination v-model:page="change_log_page" :page-count="change_log_pageCount" show-size-picker :page-sizes="change_log_pageSizes" />
        </div>
      </template>
    </ModalDialog>

    <ModalDialog ref="show_password_modalDialog" title="查看管理账户信息?" :style="{ height: '500px', width: '500px' }" @confirm="second_onConfirm">
      <template #content>
        <n-space>
          <h3>请输入二级密码</h3>
          <n-input
    ...
    <ModalDialog ref="bind_ip_modalDialog" title="关联IP" :style="{ height: '400px', width: '300px' }" @confirm="BindIpConfirm">
      <template #content>
        <n-form
    ...
    <ModalDialog ref="connect_account_modalDialog" title="关联设备管理账户" :style="{ height: '500px', width: '500px' }" @confirm="ConnectAccountConfirm">
      <template #content>
        <DataForm
    ...
    <ModalDialog ref="account_modalDialog" title="账户信息" :style="{ height: '500px', width: '800px' }">
      <template #content>
        <n-data-table
          :data="account_list.slice((account_page - 1) * account_pageSize, account_page * account_pageSize)"
    ...
    <n-modal v-model:show="collect_modalDialog" class="modal-dialog-wrapper" header-style="padding: 10px 20px" :mask-closable="false" :style="bodyStyle">
      <n-card style="width: 600px" title="关联采集方案" :bordered="false" size="huge" role="dialog" aria-modal="true">
        ...
        >
          ...
          <n-form-item label="采集方案">
            <n-select v-model:value="selectCollectValues" filterable placeholder="勾选采集方案" :options="collection_options" />
          </n-form-item>
        </n-form>
        <template #footer>
          <div class="flex justify-end">
  ...
  setup() {
                      ...
                      () =>
                        h(
                          ...
                          ' ' +
                            ...
                            '/' +
                            (parseInt(int_used_obj.value['int_used_1g']) + parseInt(int_used_obj.value['int_unused_1g']))
                        )
                    ),
                    h(
                      NFormItem,
                      ...
                      () =>
                        h(
                          ...
                          '' +
                            ...
                            '/' +
                            (parseInt(int_used_obj.value['int_used_10g']) + parseInt(int_used_obj.value['int_unused_10g']))
                        )
                    ),
                    h(
                      NFormItem,
                      ...
                      () =>
                        h(
                          ...
                          ' ' +
                            ...
                            '/' +
                            (parseInt(int_used_obj.value['int_used_40g']) + parseInt(int_used_obj.value['int_unused_40g']))
                        )
                    ),
                    h(
                      NFormItem,
                      ...
                      () =>
                        h(
                          ...
                          '' +
                            ...
                            '/' +
                            (parseInt(int_used_obj.value['int_used_100g']) + parseInt(int_used_obj.value['int_unused_100g']))
                        )
                    ),
                    h(
                      NFormItem,
                      ...
                      () => h('span', {}, '' + int_used_obj.value['int_used'] + '/' + parseInt(int_used_obj.value['int_total']))
                    )
                  ]
                ),
                h(
    ...
    function NewConnectCollect() {
      ...
      checkedRowKeysRef.value.forEach((devid) => {
        const collect_data = new FormData()
        const dev_detail_info = table.dataList.filter((item) => item.id === devid)[0]
    ...
    function NewConnectReport() {
      ...
      checkedRowKeysRef.value.forEach((devid) => {
        const report_data = new FormData()
        const dev_detail_info = table.dataList.filter((item) => item.id === devid)[0]
    ...
    function doRefresh() {
      ...
      let request_url = ''
      const search_form = searchDataFormRef.value
      const info_host_list = sessionStorage.getItem('hostlist')
    ...
    function doIdc() {
      get({
        ...
        for (let i = 0; i < idc_list.length; i++) {
          const dict = {
    ...
    function importDataFormConfirm() {
      if (importDataFormRef.value?.validator()) {
        ...
        const endyear = myDate.getFullYear() + 3 //获取当前年
        const mon = myDate.getMonth() + 1 //获取当前月
        const date = myDate.getDate()
        device_import_modalDialog.value?.toggle()
        const import_form = importDataFormRef.value.generatorParams()
        //console.log('设备录入参数', import_form)
        const import_formdata = new FormData()
        ...
        const vendor_id = import_form['vendor']
        if (vendor_id === 2) {
    ...
    function EditConfirm() {
      ...
      const device_info = EditDataFormRef.value.generatorParams()
    ...
    function export_excel() {
      ...
      if (checkedRowKeysRef.value.length > 0) {
        ...
        let export_str = '设备名称,管理IP,序列号,厂商,型号,类型,机房位置,机房模块,区域,角色,状态,机柜,U位\n'
    ...
    function ConnectAccountConfirm() {
      ...
      const connect_account_info = connect_account_DataFormRef.value?.generatorParams()
      const account_data = new FormData()
