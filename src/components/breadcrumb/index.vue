  ...
  <n-breadcrumb>
    <transition-group name="breadcrumb">
      <n-breadcrumb-item v-for="item of breadcrumbs" :key="item.key">
        <n-dropdown v-if="item.children && item.children.length > 0" :options="item.children" @select="handleSelect">
          <span>
            {{ item.label }}
            <n-icon>
              <ChevronDown />
...
export default defineComponent({
  ...
  setup() {
    ...
    function generatorDropdown(routes: Array<RouteRecordRawWithHidden> | undefined, parentPath = '/') {
      if (!routes) return
