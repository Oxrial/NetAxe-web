...
import { ADMIN_WORK_SETTING_INFO, ADMIN_ROUTES, ADMIN_WORK_S_TENANT } from '@/store/keys'
...
const store: StoreType = {
  ...
  // persist: {
  //   enabled: true,
  //   strategies: [{ storage: localStorage, paths: ['permissionRoutes'] }]
  // },
  ...
  initPermissionRoute(routes: Array<RouteRecordRaw>) {
    ...
    // console.log(this.state.permissionRoutes);
    localStorage.setItem(ADMIN_ROUTES, JSON.stringify(tempRoutes))
  },
  isEmptyPermissionRoute() {
    if (!(!this.state.permissionRoutes || this.state.permissionRoutes.length === 0)) {
      const routes = localStorage.getItem(ADMIN_ROUTES)
      if (routes) {
        this.state.permissionRoutes.splice(0, this.state.permissionRoutes.length)
        this.state.permissionRoutes.push(...JSON.parse(routes))
      }
    }
